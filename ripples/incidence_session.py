...
for id in subjects.subjects:
    ...
    for task in subjects.sessions:
        for session in subjects.sessions[task]:
            dfs.append(pd.read_csv(os.path.join(subject[task + 'dir'], session, 'ripple_incidence.csv')))
...
sns.barplot(data=df[(df['task'] == 'hab') & (df['session'] != 'Sleep3')], y='density_60', x='strain', hue='session', errorbar='se')
plt.show()
sns.barplot(data=df[(df['task'] == 'olm') & (df['session'] != 'Sleep3')], y='density_60', x='strain', hue='session', errorbar='se')
plt.show()
