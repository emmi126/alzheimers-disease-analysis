...
assemblies = pickle.load(open('../output/assemblies_' + bin_size +'.pkl', 'rb'))

for strain in assemblies:

    for i in range(0, len(assemblies[strain])):
        assembly = assemblies[strain][i]
        session = assembly['session']

        if session != 'Hab Rec 1' and session != 'OLM Rec 1':
            continue

        post_assembly = assemblies[strain][i+1]
