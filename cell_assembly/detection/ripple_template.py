...
new = True
if os.path.exists('../../output/assemblies_ripples.pkl') and new == False:
    assemblies = pickle.load(open('../../output/assemblies_ripples.pkl', 'rb'))
else:
    for strain in files:
        for k in files[strain]:
            ...
            for d in range(0,len(directories)):
                ...
                # find templates from ripple bins
                z = np.take(z_matrix, r_bins, 1)

                templates = unit_utils.assembly_templates_ica(z)
                ...
                strength_hab_s1 = unit_utils.assembly_detection(templates, z_matrix)
    ...
    pickle.dump(assemblies, open('../../output/assemblies_ripples.pkl', 'wb'))