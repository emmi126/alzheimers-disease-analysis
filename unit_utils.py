...
if __name__ == "__main__":
    from subjects_OLD import d

    ts = d['hab']['Sleep1']['ts'] # see sessions in subject.py
    spike_matrix_result, bins = spike_matrix(ts, d['hab']['pyr_units'], 20)
    # units: hab/olm, pyr_units/int_units - pyr are pyramidal cells (excitatory), int are interneurons (inhibitory)
    # an unit is a neuron
    print(spike_matrix_result)
    print(bins)

# binsize in ms
# TODO: deal with nan somehow
def z_spike_matrix(ts, units, binsize): ### USE THIS - NORMALIZES/Z-SCORES THIS
